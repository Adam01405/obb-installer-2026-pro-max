...
### v2.2 · 2026-08-29 — 去广告工具箱

- **去广告工具箱** — 在应用内集成 ApkAdRemover 去广告引擎（工具箱页）：去除广告、
  解除签名校验（普通 / 原包模式）、修补 Flutter `libapp.so`、优化 DEX，并提供
  签名模式、跳过重签名、输出目录、特征文件等开关。
- **统一输出目录** — 处理产物、`ad_patterns.json` 配置与订阅文件默认保存到
  `Download/OBBInstaller/`。
- **导出兜底** — 结果优先保存在原 APK 所在目录，其次 SAF，未授予存储权限时通过
  MediaStore 保存到 `Download/OBBInstaller/`。
- **内存与日志修复** — 处理日志设置上限，防止无限增长；非法订阅口令会给出明确错误
  提示，不再误报成功。
- **关于 / 帮助页** — 新增可折叠的工具箱、功能、隐私与免责声明分区。

### v2.1 · 2026-08-28 — 34 种语言与稳定性修复

- **34 种界面语言**（原 10 种），跟随系统语言。
- **修复签名校验** — 此前 hub keystore 被当作 JKS 加载（实际为 PKCS12），
  签名不一致从不提示；现在同版/新版重装会先提醒卸载旧应用。
- **修复内置模式崩溃** — 读取压缩的内置 APK/OBB 曾抛 `IOException`；
  现在优雅降级并正常安装。
- **修复安装卡死** — `PackageInstaller` 广播丢失会让界面停在「正在安装…」；
  现在 10 分钟超时返回明确错误。
- **修复导出路径提示** — MediaStore 会自动重命名重复文件；「已保存到…」
  现在显示磁盘上的真实文件名。
